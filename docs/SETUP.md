...
| A single test class fails when run with `--filter`, but the full suite passes | All classes share one `[Collection("api")]` DB, so a class can accidentally rely on data seeded by sibling classes. `AdminAnalyticsTests` and `AdminStudentsTests` are now self-contained (they seed the steps/data they assert on), so `--filter` runs of those two are trustworthy; other classes may still carry latent order-dependence | For a class not in that list, run the full suite to confirm before trusting a filtered failure; the shared collection is intentional (one DB rebuild per run) |
