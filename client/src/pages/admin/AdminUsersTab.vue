...
const loadUsers = () => {
  props.api.get<User[]>('/users').then((data) => { users.value = data; }).catch(() => {});
};
...
const handleSubmit = async () => {
  ...
  try {
    if (editingId.value) {
      await props.api.put(`/users/${editingId.value}`, { displayName: form.value.displayName, role: form.value.role });
    } else {
      await props.api.post('/users', { email: form.value.email, displayName: form.value.displayName, role: form.value.role });
    }
    resetForm();
    loadUsers();
  } catch (err: any) {
  ...
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="font-body text-xs text-csub-gray mt-1">Manage who can access the admin portal and their permission level</p>
      </div>
      <button
        v-if="!showForm"
    ...
    <div class="space-y-2">
      ...
      >
        <div class="flex items-center gap-4">
          <div class="w-10 h-10 rounded-full bg-csub-blue/10 flex items-center justify-center font-display text-sm font-bold text-csub-blue-dark">
          ...
          <span :class="`text-xs px-2 py-0.5 rounded-full font-body font-medium ${ROLE_COLORS_LIGHT[user.role] || 'bg-gray-100 text-gray-600'}`">
