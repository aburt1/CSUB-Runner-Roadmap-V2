...
const existingContact = props.step?.contact_info
  ? (typeof props.step.contact_info === 'string' ? JSON.parse(props.step.contact_info) : props.step.contact_info)
  : {};
...
const handleSubmit = () => {
  const contactInfo = (contactName.value || contactEmail.value || contactPhone.value)
    ? { name: contactName.value || null, email: contactEmail.value || null, phone: contactPhone.value || null }
    : null;
...
<template>
  <form @submit.prevent="handleSubmit" class="space-y-4 bg-white border border-gray-200 rounded-xl p-5">
    ...
    <section class="bg-gray-50/70 border border-gray-200 rounded-xl p-4 space-y-4">
      <div>
        ...
        <p class="font-body text-xs text-csub-gray mt-1">Start with the content and icon students will see.</p>
      </div>
      <div class="grid grid-cols-1 sm:grid-cols-[1fr_auto] gap-4 items-start">
        ...
        <div class="sm:w-48">
          ...
          <div class="relative" ref="iconPickerRef">
            <div class="relative">
              <span class="absolute left-3 top-1/2 -translate-y-1/2 w-8 h-8 rounded-md bg-gray-50 border border-gray-200 flex items-center justify-center text-xl pointer-events-none">
                {{ icon || '📋' }}
              </span>
              <input type="text" value="" @focus="showEmojiPicker = true" :class="`${field} pl-14`" :placeholder="icon ? 'Change emoji' : 'Choose emoji'" readonly />
            </div>
      ...
      <div>
        ...
        <input type="text" v-model="description" :class="field" placeholder="One sentence that explains what the student needs to do." />
      </div>
      <div>
    ...
    <section class="bg-gray-50/70 border border-gray-200 rounded-xl p-4 space-y-4">
      <div>
        ...
        <p class="font-body text-xs text-csub-gray mt-1">Describe who should see this step. The app still uses tags underneath.</p>
      </div>
      ...
      <div>
        ...
        <p class="font-body text-xs text-csub-gray mb-2">Leave empty to show the step to everyone in the term.</p>
      ...
      <div v-if="requiredTags.length > 1" class="space-y-3">
        <div>
          <p class="font-body text-xs font-semibold text-csub-blue-dark">If a student has multiple audience tags</p>
          <p class="font-body text-xs text-csub-gray mt-1">Choose whether matching one tag is enough, or whether they need every selected tag.</p>
        </div>
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <button type="button" @click="requiredTagMode = 'any'" :class="`text-left rounded-xl border px-4 py-3 transition-colors ${requiredTagMode === 'any' ? 'border-csub-blue bg-csub-blue/5 ring-1 ring-csub-blue' : 'border-gray-300 bg-white hover:border-csub-blue/40'}`">
            <div class="flex items-center justify-between gap-3">
              <p class="font-body text-sm font-semibold text-csub-blue-dark">Match any selected tag</p>
              <span v-if="requiredTagMode === 'any'" class="font-body text-xs font-semibold text-csub-blue">Active</span>
            </div>
            <p class="font-body text-xs text-csub-gray mt-2">A student sees this step if they match at least one of the selected tags.</p>
          </button>
          <button type="button" @click="requiredTagMode = 'all'" :class="`text-left rounded-xl border px-4 py-3 transition-colors ${requiredTagMode === 'all' ? 'border-csub-blue bg-csub-blue/5 ring-1 ring-csub-blue' : 'border-gray-300 bg-white hover:border-csub-blue/40'}`">
            <div class="flex items-center justify-between gap-3">
              <p class="font-body text-sm font-semibold text-csub-blue-dark">Match every selected tag</p>
              <span v-if="requiredTagMode === 'all'" class="font-body text-xs font-semibold text-csub-blue">Active</span>
            </div>
            <p class="font-body text-xs text-csub-gray mt-2">A student only sees this step if they match all of the selected tags.</p>
          </button>
        </div>
      ...
      <div v-else class="bg-white border border-dashed border-gray-300 rounded-lg px-3 py-2">
        <p class="font-body text-xs text-csub-gray">Add two or more audience tags if you want to choose between matching one tag or all tags.</p>
      </div>
      <div>
        <button type="button" @click="showAdvancedRules = !showAdvancedRules" class="font-body text-sm font-semibold text-csub-blue hover:text-csub-blue-dark transition-colors">
      ...
      <div v-if="showAdvancedRules">
        ...
        <p class="font-body text-xs text-csub-gray mb-2">Students with any of these tags will not see this step.</p>
        <div class="flex flex-wrap gap-1.5 mb-2">
          <button
            v-for="tag in TAG_PRESETS"
            :key="tag"
    ...
    <section class="bg-gray-50/70 border border-gray-200 rounded-xl p-4 space-y-4">
      <div>
        ...
        <p class="font-body text-xs text-csub-gray mt-1">Secondary options for timing and preview behavior.</p>
      </div>
      ...
      <label class="flex items-center gap-2 cursor-pointer">
        <input type="checkbox" v-model="isPublic" class="w-4 h-4 rounded border-gray-300 text-csub-blue focus:ring-csub-blue" />
        <span class="font-body text-sm text-csub-blue-dark">Show in the public preview before login</span>
      </label>
      <label class="flex items-center gap-2 cursor-pointer">
        <input type="checkbox" v-model="isOptional" class="w-4 h-4 rounded border-gray-300 text-csub-blue focus:ring-csub-blue" />
    ...
    <section class="bg-gray-50/70 border border-gray-200 rounded-xl p-4 space-y-4">
      <div>
        ...
        <p class="font-body text-xs text-csub-gray mt-1">Optional contact details shown to students on this step.</p>
      </div>
    ...
    <div class="flex gap-3 pt-2">
      <button type="submit" class="bg-csub-blue hover:bg-csub-blue-dark text-white font-display font-bold uppercase tracking-wider px-6 py-2.5 rounded-lg shadow transition-colors text-sm">
        {{ step ? 'Save Changes' : 'Create Step' }}
      </button>
      <button type="button" @click="emit('cancel')" class="border border-gray-300 text-csub-gray hover:text-csub-blue-dark font-body px-6 py-2.5 rounded-lg transition-colors text-sm">
        Cancel
      </button>
    </div>
  </form>
