...
const handleTermsChange = (nextTerms: Term[], preferredTermId: number | null = selectedTermId.value) => {
...
watch(
  ...
  () => {
    ...
    api.value.get<Term[]>('/terms').then((data) => {
...
watch(
  ...
  () => {
    ...
    api.value.get<Step[]>(`/steps?term_id=${selectedTermId.value}`).then((data) => { steps.value = data; }).catch(() => {});
  },
  { immediate: true },
);
...
<template>
  ...
  <div v-else class="min-h-screen bg-gray-50">
    ...
    <div class="bg-csub-blue-dark text-white">
      <div class="max-w-5xl mx-auto px-4 sm:px-6 py-4">
        ...
        <div class="flex items-center justify-between">
          <h1 class="font-display text-lg font-bold uppercase tracking-wide">
            Rowdy Rundown
          </h1>
          <div class="flex items-center gap-3 text-sm font-body">
            <label v-if="showHeaderTermSelector" class="flex items-center gap-2 text-xs text-white/70">
    ...
    <div class="bg-white border-b border-gray-200 shadow-sm">
      <div class="max-w-5xl mx-auto px-4 sm:px-6 flex gap-1 py-1.5 overflow-x-auto" role="tablist" aria-label="Admin sections">
        <button
          v-for="tab in tabs"
          :key="tab.key"
          role="tab"
        ...
        >
          <svg v-if="tab.key === 'students'" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" :stroke-width="2">
            <path stroke-linecap="round" stroke-linejoin="round" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z" />
          </svg>
          <svg v-else-if="tab.key === 'termSteps'" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" :stroke-width="2">
            <path stroke-linecap="round" stroke-linejoin="round" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4" />
          </svg>
          <svg v-else-if="tab.key === 'analytics'" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" :stroke-width="2">
            <path stroke-linecap="round" stroke-linejoin="round" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
          </svg>
          <svg v-else-if="tab.key === 'audit'" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" :stroke-width="2">
            <path stroke-linecap="round" stroke-linejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
          </svg>
          <svg v-else-if="tab.key === 'terms'" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" :stroke-width="2">
            <path stroke-linecap="round" stroke-linejoin="round" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
          </svg>
          <svg v-else-if="tab.key === 'users'" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" :stroke-width="2">
    ...
    <!-- Tab content -->
    <div class="max-w-5xl mx-auto px-4 sm:px-6 py-8" role="tabpanel" :id="`tabpanel-${activeTab}`" :aria-labelledby="`tab-${activeTab}`">
      <StudentsTab v-if="activeTab === 'students'" :api="api" :steps="steps" :role="role" :term-id="selectedTermId" />
      <TermStepsTab
