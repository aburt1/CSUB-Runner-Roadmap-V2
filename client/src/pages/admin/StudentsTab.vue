    ...
    <div class="grid grid-cols-1 md:grid-cols-[1fr_2fr] gap-6">
      ...
      <div>
        <div class="mb-4">
          <div class="flex items-center gap-3">
            ...
            <span
              ...
              class="font-body text-xs text-csub-gray bg-gray-100 rounded-full px-2.5 py-0.5"
              >{{ totalStudents }} total</span
            >
            <div class="flex-1 h-px bg-gray-200" />
          </div>
          <p class="font-body text-xs text-csub-gray mt-1">
            Select a student to view their progress and manage step completions
          </p>
        </div>
        ...
        <div class="flex items-center gap-2 mb-3">
          ...
          <button
            @click="handleOverdueToggle"
            :class="`flex items-center gap-1.5 px-3 py-1.5 rounded-lg border font-body text-xs font-medium transition-all ${overdueOnly ? 'border-red-300 bg-red-50 text-red-700' : 'border-gray-300 bg-white text-csub-gray hover:border-red-200 hover:text-red-600'}`"
          >
            <svg
              class="w-3.5 h-3.5"
              fill="none"
        ...
        <div class="space-y-2 max-h-[55vh] overflow-y-auto pr-1">
          <p
            ...
            class="font-body text-sm text-csub-gray text-center py-6"
          >
            ...
            <div class="flex items-center gap-3">
              ...
              <div class="flex-1 min-w-0">
                ...
                <div class="flex flex-wrap gap-1 mt-1">
                  <span
                    v-if="s.applicant_type"
                    class="text-[10px] bg-gray-100 text-csub-blue-dark px-1.5 py-0.5 rounded-full font-body"
                    >{{ s.applicant_type }}</span
                  >
                  <span
                    v-if="s.residency"
                    class="text-[10px] bg-gray-100 text-csub-blue-dark px-1.5 py-0.5 rounded-full font-body"
        ...
        >
          <button
            ...
            :disabled="page <= 1"
            class="flex items-center gap-1 font-body text-xs font-semibold text-csub-blue hover:text-csub-blue-dark disabled:text-gray-300 disabled:cursor-not-allowed transition-colors"
          >
            <svg
              class="w-4 h-4"
              fill="none"
      ...
      <div class="md:sticky md:top-4 md:self-start">
        <StudentDetail
          ...
          :api="api"
          :role="role"
          @progress-change="refreshStudents"
        />
      </div>
    </div>
  </div>
