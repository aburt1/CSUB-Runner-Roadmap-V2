...
watch(
  ...
  () => {
    const fetchData = async () => {
      try {
        const result = await props.api.get<RiskStep[]>('/analytics/deadline-risk', { term_id: props.termId, days: 14 });
        data.value = result;
      } catch (err) {
...
<template>
  ...
  <div v-else class="bg-white border border-gray-200 rounded-xl shadow-sm p-5">
    ...
    <p v-if="data.length === 0" class="font-body text-sm text-csub-gray">No steps with upcoming deadlines.</p>
    <div v-else class="overflow-x-auto">
      <table class="w-full text-sm font-body">
        <thead>
          <tr class="border-b border-gray-200">
            <th class="text-left py-2 px-3 font-semibold text-csub-blue-dark uppercase text-xs tracking-wide">Step</th>
            <th class="text-left py-2 px-3 font-semibold text-csub-blue-dark uppercase text-xs tracking-wide">Deadline</th>
            <th class="text-center py-2 px-3 font-semibold text-csub-blue-dark uppercase text-xs tracking-wide">At Risk</th>
          </tr>
        </thead>
        <tbody>
          <tr
