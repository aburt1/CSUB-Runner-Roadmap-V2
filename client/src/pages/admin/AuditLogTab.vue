...
const fetchLogs = async (filters: FetchFilters = {}, off = 0, append = false) => {
  ...
  try {
    ...
    const data = await props.api.get<{ logs: AuditLog[]; total: number }>(
...
const handleStudentSearch = async () => {
  ...
  try {
    const data = await props.api.get<{ students?: StudentResult[] }>(
      `/students?search=${encodeURIComponent(studentSearch.value)}`,
    )
...
<template>
  <div>
    ...
    <div v-if="logs.length === 0 && !loading" class="text-center py-10">
      <p class="font-body text-sm text-csub-gray">No audit entries match the current filters.</p>
    </div>
