    ...
    public async Task<IActionResult> ListStudents()
    {
        ...
        var overdueJoin = $@"
            LEFT JOIN (
              ...
              JOIN steps st ON st.{QueryHelpers.ActiveStepFilter} AND st.deadline_date IS NOT NULL AND st.deadline_date < CONVERT(varchar(10), CAST(SYSUTCDATETIME() AT TIME ZONE 'Pacific Standard Time' AS date), 23)
